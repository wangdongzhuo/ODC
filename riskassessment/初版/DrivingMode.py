...
def get_driving_mode():
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            result["status"] = "error"
            result["message"] = f"获取风险数据失败: {response.status_code}"
    except Exception as e:
        result["status"] = "error"
        result["message"] = f"调用风险API异常: {str(e)}"
    ...
    try:
        ...
        if all(r.status_code == 200 for r in responses):
            object_state = response1.json()
            road_information_state     = response2.json()
            weather_state              = response3.json()
            road_infrucstrcution_state = response4.json()
            digtal_state               = response5.json()
            tran_condi_state           = response6.json()
            communica_condi_state      = response7.json()
            driver_state               = response8.json()
            vehicle_state              = response9.json()
            map_local_state            = response10.json()
            # ODD元素
            result["data"]["odd_elements"] = {
                "object_flag": object_state[1],
                "road_information_flag": road_information_state[1],
                "weather_flag": weather_state[1],
                "road_infrucstrcution_flag": road_infrucstrcution_state[1],
                "digtal_flag": digtal_state[1],
                "tran_condi_flag": tran_condi_state[1],
                "communica_condi_flag": communica_condi_state[1],
                "driver_flag": driver_state[1],
                "vehicle_flag": vehicle_state[1],
                "map_local_flag": map_local_state[1]
            }
        else:
            result["status"] = "error"
            result["message"] = "部分ODD元素数据获取失败"
            for i, r in enumerate(responses):
                if r.status_code != 200:
                    result["message"] += f", API {i+1}: {r.status_code}"    
    except Exception as e:
        result["status"] = "error"
        result["message"] = f"调用ODD元素API异常: {str(e)}"
