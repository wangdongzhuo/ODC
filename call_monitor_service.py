...
from ODC_data.main.call_api_traffic import get_traffic_status
...
jsonpath = os.path.join(root_path, "monitorODC\\ODC_data\\main") 
    ...
    for var_name, (first_cacategory,second_category, var_value) in weather_information.items():
        ...
        if type(var_value) in (int, float):
            try:
                ...
                min_val, max_val = ODC[first_cacategory][second_category][""]["value"]
            status = "正常范围" if is_valid else "过大" if var_value > max_val else "过小"
            result_status.append([first_cacategory,second_category,var_name,var_value, "green" if is_valid else "red", status])
        # elif type(var_value) in (bool):
        elif type(var_value) == bool:
            result_status.append([first_cacategory,second_category,var_name,var_value, "green" if is_valid else "red", "正常" if is_valid else "异常"])
        # elif type(var_value) in (str):
...
#         return None
    ...
    #     center_latitude =   28.190630091477217    # 圆心纬度
    #     center_longitude =  106.1871641414752596  # 圆心经度
    #     result = get_traffic_status(center_latitude, center_longitude)
    #     print(result)

@app.route('/get_object_state', methods=['GET'])
def get_object_state():
    object={'目标物':{'目标物类型':'行人'}}
    object_info_str,ODD_state,element=check_object_rule(object)
    return jsonify(object_info_str,ODD_state,element)

@app.route('/get_road_information_state', methods=['GET'])
def get_road_information_state():
    coordinates = [(106.363359, 29.568271)]
    json_file = os.path.join(jsonpath, "map_data.json")
    road_information=query_road_info(coordinates[0][0],coordinates[0][1], json_file = json_file)
    road_str,ODD_state,element=check_road_attribution(road_information)
    return jsonify(road_str,ODD_state,element)

@app.route('/get_weather_state', methods=['GET'])
def get_weather_state():
    weather_data=get_weather_data()
    weather_str,ODD_state,element=check_weather_condition(weather_data)
    return jsonify(weather_str,ODD_state,element)

@app.route('/get_road_infrucstrcution_state', methods=['GET'])
def get_road_infrucstrcution_state():
    coordinates = [(106.363359, 29.568271)]
    json_file = os.path.join(jsonpath, "map_data.json")
    road_information=query_road_info(coordinates[0][0],coordinates[0][1], json_file)
    road_infrucstrcution_str,ODD_state,element=check_road_infrustruction(road_information)
    return jsonify(road_infrucstrcution_str,ODD_state,element)

@app.route('/get_digtal_state', methods=['GET'])
def get_digtal_state():
    digtal={'数字信息':{'消息体':True,'信源发送频率':True}}
    digtal_info_str,ODD_state,element=check_digtal_info_rule(digtal)#可检查部分
    return jsonify(digtal_info_str,ODD_state,element)

@app.route('/get_tran_condi_state', methods=['GET'])
def get_tran_condi_state():
    center_latitude = 39.98641364  # 圆心纬度
    center_longitude =  116.3057764  # 圆心经度
    result=get_traffic_status(center_latitude, center_longitude)
    traffic_status='缓行'
    traffic_event=True
    if result['trafficinfo']['evaluation']['status']== '1':
        traffic_status='畅通' #交通拥堵度

    traffic_event_cloud = get_event_info()
    if traffic_event_cloud[0]['UUID']==' ':
        traffic_event=False

    tran_condi={'交通条件':{'交通事件':traffic_event,'交通拥堵度':traffic_status,'可协作车辆':True,'交通法规':True}}
    tran_condi_info_str,ODD_state,element=check_tran_condi_rule(tran_condi)#可检查部分
    return jsonify(tran_condi_info_str,ODD_state,element)

@app.route('/get_communica_condi_state', methods=['GET'])
def get_communica_condi_state():
    # communica_condi={'通讯与计算':{'丢包率':0.01,'通信模式':'V2N','通信时延':{'协同自适应巡航':25,'无信号匝道的转向或交替通行':10,'协作式变道':0,'路口对向行驶车辆的无保护左转':0,
    #                                                         '协作式车道汇入':25,'编队行驶':25,'动态车速限制':15,'远程控制驾驶':15}}}
    result=get_channel_state()
    communica_condi={'通讯与计算':{'丢包率':result['latest_loss_rate'],'通信模式':'V2N','通信时延':result['latest_latency']}}
    communica_condi_info_str,ODD_state,element=check_communica_rule(communica_condi)#可检查部分
    return jsonify(communica_condi_info_str,ODD_state,element)

@app.route('/get_driver_state', methods=['GET'])
def get_driver_state():
    driver_data=get_driver_data()
    driver_str,ODD_state,element=check_driver_condition(driver_data)
    return jsonify(driver_str,ODD_state,element)

@app.route('/get_vehicle_state', methods=['GET'])
def get_vehicle_state():
    vehicle= get_vehicle_data()
    vehicle_run_str,ODD_run,element_run=check_vehicle_runcondition(vehicle)
    return jsonify(vehicle_run_str,ODD_run,element_run)

@app.route('/get_map_local_state', methods=['GET'])
def get_map_local_state():
    map_local_info_str,ODD_state,element=check_map_local_rule()
    return jsonify(map_local_info_str,ODD_state,element)


# @app.route('/RoutineOdd', methods=['GET'])
# def RoutineOdd(routine):
#     #整条路上的ODD颜色就根据路点的信息去检查是不是在ODD边界内，然后局部的话就调用接口，未来一段路上的比较，以最终的结果作为标色的依据，
#     #road_data假定是以列表的形式给出{[x1,y1],[x2,y2],...}
#     whole_ODD=[]
...
#         road_str,road_ODD_attribute=check_road_attribution(road_information)
#         whole_ODD.append({routine[0][0],routine[0][1],road_ODD_attribute})
#     return jsonify(whole_ODD)

if __name__ == '__main__':
    print("[信息] 启动ODC元素状态监测 API 服务")
